...
public class Kmeans {
    ...
    public void updateCentroids(int numWorkers) throws InterruptedException {
        ...
        DoubleAccumulator[][] centroidsSum = new DoubleAccumulator[numClusters][dim];
        AtomicInteger[] counts = new AtomicInteger[numClusters];
        for (int c = 0; c < numClusters; c++) {
            counts[c] = new AtomicInteger(0);
            for (int d = 0; d < dim; d++) {
                centroidsSum[c][d] = new DoubleAccumulator(Double::sum, 0.0);
            }
        }
        ...
        for (int i = 0; i < numWorkers; i++) {
            ...
            threads[i] = Thread.ofPlatform().start(() -> {
                ...
                for (int c = 0; c < numClusters; c++) {
                    if (localCounts[c] > 0) {
                        counts[c].addAndGet(localCounts[c]);
                        for (int d = 0; d < dim; d++) {
                            centroidsSum[c][d].accumulate(localSums[c][d]);
                        }
                    }
                }
            });
        ...
        for (int c = 0; c < numClusters; c++) {
            int totalCount = counts[c].get();

            if (totalCount > 0) {
                double[] finalCoordinates = new double[dim];
                for (int d = 0; d < dim; d++) {
                    finalCoordinates[d] = centroidsSum[c][d].get() / totalCount;
                }
                this.centroids[c] = new Point(finalCoordinates);
            }
        }
    }
